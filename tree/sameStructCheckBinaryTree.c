...
struct binaryTreeNode *queueFront(struct Queue *q) 
{
    if (q->front == NULL)
        return NULL;
...
struct binaryTreeNode * queueRear(struct Queue *q) 
{
    if (q->rear == NULL)
        return NULL;
...
void _destoryBinaryTree(struct binaryTreeNode *root) {

    if (root == NULL)
        return;

    //first delete both subtrees
    _destoryBinaryTree(root->left);
    _destoryBinaryTree(root->right);
...
*/
void destoryBinaryTree(struct binaryTreeNode **root_ref)
{
    _destoryBinaryTree(*root_ref);
    *root_ref = NULL;
}

/*
Check binary is the same or not 

Time Complexity: O(n)
Space Complexity: O(n), for recursive stack
*/
int sameStructBt(struct binaryTreeNode *root1, struct binaryTreeNode *root2) {
    ...
    // both non-empty 
    return (root1->data == root2->data && sameStructBt(root1->left, root2->left) && \
                  sameStructBt(root1->right, root2->right)); 
}
